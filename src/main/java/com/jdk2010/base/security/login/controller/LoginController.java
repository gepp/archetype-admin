...
public class LoginController extends BaseController {
	...
	public String in(HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		String username = CookieUtil.getCookieValue(request, response,
				"username");
		String md5Password = CookieUtil.getCookieValue(request, response,
				"md5Password");
	...
	public void inAction(HttpServletRequest request,
		...
		SecurityUser securityUser = securityUserService.login(username,
				password);
		if (securityUser == null) {
			...
			if ("true".equals(rememberMe)) {
				CookieUtil.addCookie(request, response, "username", username,
						60 * 60);
				CookieUtil.addCookie(request, response, "md5Password",
						password, 60 * 60);
			}
